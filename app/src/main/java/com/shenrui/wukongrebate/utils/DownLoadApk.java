...
public class DownLoadApk {
    ...
    public static void download(Context context, String url, String title,final String appName,final String versionName) {
        // 获取存储ID
        ...
        if (downloadId != -1L) {
            ...
            if (status == DownloadManager.STATUS_SUCCESSFUL) {
                ...
                start(context, url, title,appName,versionName);
    ...
    private static void start(Context context, String url, String title,String appName,String versionName) {
        long id = FileDownloadManager.getInstance(context).startDownload(url,title, "下载完成后点击打开",appName,versionName);
        SharedPreferences sp = PreferenceManager.getDefaultSharedPreferences(context);
        sp.edit().putLong(DownloadManager.EXTRA_DOWNLOAD_ID,id).commit();
        sp.edit().putString("nerVersion",versionName).commit();
